...
namespace SmartWinControls.SmartControls.Forms
{
    public partial class SmartForm : Form
    {
        ...
        #region editable smartform properties
        ...
        public void ActivateMdiChildForm(Form childToActivate)
        {
            ...
            WinAPI.SendMessage(mdi.Handle,
                (int)WinAPI.WindowsMessages.WM_MDINEXT, form.Handle, next);            
        }
        ...
        protected override void WndProc(ref Message m)
        {
            ...
            switch (m.Msg)
            {
                case (int)WinAPI.WindowsMessages.WM_NCCALCSIZE: //当计算某个窗口的客户区大小和位置时发送此消息
                    alreadyHandled = WmNcCalcSize(ref m);
                    break;

                case (int)WinAPI.WindowsMessages.WM_NCHITTEST://移动鼠标，按住或释放鼠标时产生此消息
                    alreadyHandled = WmNcHitTest(ref m);
                    break;

                case (int)WinAPI.WindowsMessages.WM_NCACTIVATE:////通过改变某个窗口的非客户区来表示窗口是处于激活还是非激活状态时，此消息被发送给该窗口
                    alreadyHandled = WmNcActivate(ref m);
                    break;

                case (int)WinAPI.WindowsMessages.WM_NCPAINT://当某个窗口的框架必须被绘制时，应用程序发送此消息给该窗口
                    alreadyHandled = true;
                    break;
